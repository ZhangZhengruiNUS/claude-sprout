...
function Get-AgentDesktopCompanionRoot {
  if ($env:AGENT_DESKTOP_COMPANION_HOME) { return $env:AGENT_DESKTOP_COMPANION_HOME }
  if ($env:CLAUDE_SPROUT_HOME) { return $env:CLAUDE_SPROUT_HOME }
  $newRoot = Join-Path $env:USERPROFILE ".agent-desktop-companion"
  $legacyRoot = Join-Path $env:USERPROFILE ".claude-sprout"
  if ((Test-Path -LiteralPath $newRoot) -or -not (Test-Path -LiteralPath $legacyRoot)) {
    return $newRoot
  }
  return $legacyRoot
}
...
try {
  ...
  if ([string]::IsNullOrWhiteSpace($sessionId)) {
    Write-Output "Agent Desktop Companion: no session"
    exit 0
  }

  $root = Get-AgentDesktopCompanionRoot
  ...
  Write-Output "Agent Desktop Companion: $project | $contextText"
}
catch {
  Write-Output "Agent Desktop Companion: status unavailable"
  exit 0
}
